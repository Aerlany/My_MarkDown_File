...
>
> **`mouseenter`** 事件在定点设备（通常指鼠标）首次移动到[元素](https://developer.mozilla.org/zh-CN/docs/Web/API/Element)的激活区域内时，在该元素上触发。鼠标进入一个节点时触发，进入子节点不会触发这个事件。
>
> **`mouseover`**：鼠标进入一个节点时触发，进入子节点会再一次触发这个事件（可以出发事件冒泡）
>
...
![event](https://github.com/Aerlany/Images-of-mine/raw/main/PicGo/event.png)
...
- Web APIs :
 ...
 ![67604738945](https://github.com/Aerlany/Images-of-mine/raw/main/PicGo/1676047389456.png)
...
- window对象下的属性和方法调用的时候可以省略window
 ...
 ![67604743636](https://github.com/Aerlany/Images-of-mine/raw/main/PicGo/1676047436362.png)
...
![67604963508](https://github.com/Aerlany/Images-of-mine/raw/main/PicGo/1676049635087.png)
...
表示字符的范围，定义的规则限定在某个范围，比如只能是英文字母，或者数字等等，用表示范围
 ...
 ![67608029616](https://github.com/Aerlany/Images-of-mine/raw/main/PicGo/1676080296168.png)
...
## 正则插件
 ...
 ![67608054863](https://github.com/Aerlany/Images-of-mine/raw/main/PicGo/1676080548639.png)
...
![image-20240102141647238](https://github.com/Aerlany/Images-of-mine/raw/main/PicGo/image-20240102141647238.png)
...
<body>
    <div class="box">
        <div class="form">
            ...
            <div class="userCheck">
                <input placeholder="UserPolicy" />
                <a>verification code</a>
            </div>

            <div class="checkBox">
                <input type="checkbox" /> <span style="font-size: 15px;font-weight: 200;">Have read the user
                    instructions in detail</span>
            </div>
    ...
    <script>
        let captchaCode = 0;
        const rules = [/^(?:(?:\+|00)86)?1(?:(?:3[\d])|(?:4[5-79])|(?:5[0-35-9])|(?:6[5-7])|(?:7[0-8])|(?:8[\d])|(?:9[1589]))\d{8}$/, /^\S*(?=\S{6,})(?=\S*\d)(?=\S*[A-Z])(?=\S*[a-z])(?=\S*[!@#$%^&*? ])\S*$/];
        const inputs = document.querySelectorAll(".userCheck");
        console.log(inputs);
        for (let index = 0; index < inputs.length - 1; index++) {
            inputs[index].querySelector("input").addEventListener("change", () => {
                const child = inputs[index].childNodes;
                if (child[child.length - 1].className == 'extraA') {
                    inputs[index].removeChild(child[child.length - 1]);
                }
                if (!rules[index].test(inputs[index].querySelector("input").value) && inputs[index].querySelector("input").value != "") {
                    const a = document.createElement('a');
                    a.innerText = "Access Denny";
                    a.style.color = "red";
                    a.className = "extraA"
                    inputs[index].appendChild(a);
                }
            })
        }
        ...
        inputs[2].querySelector("a").addEventListener("click", () => {
            const child = inputs[2].childNodes;
            if (child[child.length - 1].className == 'extraDiv') {
                inputs[2].removeChild(child[child.length - 1]);
            }
            var count = 60;
            const interval = setInterval(function () {
                count--;
                inputs[2].querySelector("a").innerText = count;
                if (count == 0) {
                    inputs[2].querySelector("a").innerText = "verification code";
                    clearInterval(interval);
                }
            }, 1000);
            captchaCode = generateRandomCode();
            const captchaContainer = document.createElement("div");
            captchaContainer.innerHTML = '<img src="https://via.placeholder.com/150x50.png?text=' + captchaCode + '" alt="验证码">';
            captchaContainer.className = 'extraDiv';
            inputs[2].appendChild(captchaContainer)
        })

        inputs[2].querySelector("input").addEventListener("change", () => {
            if (inputs[2].querySelector("input").value != captchaCode) {
                const a = document.createElement('a');
                a.innerText = "Access Denny";
                a.style.color = "red";
                inputs[2].insertBefore(a,);
            }
        })
...
<body>
    <div class="main">
        ...
        <div class="login2 hidden">
            <img src="testImg" alt="二维码">
        </div>
    </div>

    <script>
        const a = document.querySelectorAll("li");
        const box1 = document.querySelector(".login1")
        const box2 = document.querySelector(".login2")
...
![67679290689](https://github.com/Aerlany/Images-of-mine/raw/main/PicGo/1676792906898.png)

### 面向对象

面向对象是把事务分解成为一个个对象，然后由对象之间分工与合作。

![67679293032](https://github.com/Aerlany/Images-of-mine/raw/main/PicGo/1676792930329.png)
...
![67679338869](https://github.com/Aerlany/Images-of-mine/raw/main/PicGo/1676793388695.png)
...
![image-20210809221226279](https://github.com/Aerlany/Images-of-mine/raw/main/PicGo/f0d87c6eb2bd4952b988bc8f855e129f~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp)
...
![image-20210809222117471](https://github.com/Aerlany/Images-of-mine/raw/main/PicGo/55eecb22956f4bd691cbea51447fc19e~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp)

深拷贝：拷贝的是对象，不是地址，深拷贝会开辟新的栈，两个对象对应两个不同的地址，修改对象A的属性，并不会影响到对象B
